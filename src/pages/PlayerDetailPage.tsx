...
import { ArrowLeft, TrendingDown, TrendingUp, Shield, ArrowUpCircle, ArrowDownCircle, Pencil, Check, X, FileDown, Star } from "lucide-react";
...
import { exportPlayerDetailPdf } from "@/utils/exportPlayerDetailPdf";
...
export default function PlayerDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) { navigate("/players"); return; }
    setPlayer(data!);
    setLoading(false);
  }, [id, navigate]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Back + Header */}
      <div>
        <Link to="/players" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors mb-4">
          <ArrowLeft size={15} />
          Players
        </Link>
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => exportPlayerDetailPdf(player)}
              className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-600 border border-slate-200 rounded-lg px-2.5 py-1.5 hover:border-slate-300 transition-colors"
              title="PDF İndir"
            >
              <FileDown size={14} />
              PDF
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Ort. Puan</p>
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="p-5">
            {player.matchPlayers.filter((mp) => !mp.match.cancelledAt).length === 0 ? (
